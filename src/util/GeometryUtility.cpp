...
namespace gaia3d
{
	...
	void GeometryUtility::mergeLegoBlocksAlongZAxis(std::vector<LegoBlock*>& legos, bool mustSameColor)
	{
		...
		while(i < legoCount)
		{
			...
			for(size_t j = i+1; j < legoCount; j++)
			{
				if(mustSameColor &&(legos[i]->color != legos[j]->color))
					continue;

				heightDifference = legos[j]->minZ - legos[i]->maxZ;
				if(fabs(heightDifference) < 0.01)
				{
					legos[i]->maxZ = legos[j]->maxZ;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
				}

				heightDifference = legos[j]->maxZ - legos[i]->minZ;
				if(fabs(heightDifference) < 0.01)
				{
					legos[i]->minZ = legos[j]->minZ;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
	...
	void GeometryUtility::mergeLegoBlocksAlongYAxis(std::vector<LegoBlock*>& legos, bool mustSameColor)
	{
		size_t legoCount = legos.size();
		size_t i = 0;
		bool merged;
		...
		while(i < legoCount)
		{
			...
			for(size_t j = i+1; j < legoCount; j++)
			{
				...
				if( fabs(legos[i]->minX - legos[j]->minX) >= 0.001 ||
					fabs(legos[i]->maxX - legos[j]->maxX) >= 0.001 ||
					fabs(legos[i]->minZ - legos[j]->minZ) >= 0.001 ||
					fabs(legos[i]->maxZ - legos[j]->maxZ) >= 0.001 )
					continue;

				yDifference = legos[j]->minY - legos[i]->maxY;
				if(fabs(yDifference) < 0.01)
				{
					legos[i]->maxY = legos[j]->maxY;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
				...
				yDifference = legos[j]->maxY - legos[i]->minY;
				if(fabs(yDifference) < 0.01)
				{
					legos[i]->minY = legos[j]->minY;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
	...
	void GeometryUtility::mergeLegoBlocksAlongXAxis(std::vector<LegoBlock*>& legos, bool mustSameColor)
	{
		size_t legoCount = legos.size();
		size_t i = 0;
		bool merged;
		...
		while(i < legoCount)
		{
			...
			for(size_t j = i+1; j < legoCount; j++)
			{
				...
				if( fabs(legos[i]->minY - legos[j]->minY) >= 0.001 ||
					fabs(legos[i]->maxY - legos[j]->maxY) >= 0.001 ||
					fabs(legos[i]->minZ - legos[j]->minZ) >= 0.001 ||
					fabs(legos[i]->maxZ - legos[j]->maxZ) >= 0.001 )
					continue;

				xDifference = legos[j]->minX - legos[i]->maxX;
				if(fabs(xDifference) < 0.01)
				{
					legos[i]->maxX = legos[j]->maxX;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
				...
				xDifference = legos[j]->maxX - legos[i]->minX;
				if(fabs(xDifference) < 0.01)
				{
					legos[i]->minX = legos[j]->minX;
					delete legos[j];
					legos.erase(legos.begin() + j);
					merged = true;
					break;
